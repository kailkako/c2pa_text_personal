...
use c2pa::Settings;
use c2pa::{
    assertions::DigitalSourceType, validation_status, Builder, BuilderIntent, Context, Error, ManifestAssertionKind, Reader,
...
fn test_builder_unsupported_format_no_embed_required() -> Result<()> {
    ...
    let mut builder = Builder::from_shared_context(&context);
    builder.set_intent(BuilderIntent::Edit);
    // no_embed is NOT set 鈥?signing should fail for an unsupported format
...
#[test]
fn test_builder_text_plain_sign_and_verify_non_ascii() -> Result<()> {
    let context = test_context().into_shared();
    let format = "text/plain";
    let source_text = "\u{8fd9}\u{662f}\u{4e00}\u{6bb5} C2PA \u{6587}\u{672c}\u{3002}\nEmoji: \u{1f980}\u{2728}\nASCII too.";
    let mut source = Cursor::new(source_text.as_bytes());
    let mut dest = Cursor::new(Vec::new());

    let mut builder = Builder::from_shared_context(&context);
    builder.set_intent(BuilderIntent::Create(DigitalSourceType::DigitalCapture));
    builder.definition.title = Some("signed-text.txt".to_string());
    builder.save_to_stream(format, &mut source, &mut dest)?;

    let signed_text = String::from_utf8(dest.get_ref().clone())
        .map_err(|err| Error::BadParam(format!("signed text is not UTF-8: {err}")))?;
    assert!(signed_text.contains(source_text));
    assert_ne!(signed_text, source_text);

    dest.rewind()?;
    let reader = Reader::from_shared_context(&context).with_stream(format, &mut dest)?;
    assert!(
        reader.validation_status().is_none(),
        "signed text should validate: {:?}",
        reader.validation_status()
    );
    assert_eq!(reader.active_manifest().unwrap().title(), Some("signed-text.txt"));

    Ok(())
}

#[test]
fn test_builder_text_plain_detects_visible_text_tamper() -> Result<()> {
    let context = test_context().into_shared();
    let format = "text/plain";
    let source_text = "Original signed text with C2PA.";
    let mut source = Cursor::new(source_text.as_bytes());
    let mut dest = Cursor::new(Vec::new());

    let mut builder = Builder::from_shared_context(&context);
    builder.set_intent(BuilderIntent::Create(DigitalSourceType::DigitalCapture));
    builder.definition.title = Some("tamper-test.txt".to_string());
    builder.save_to_stream(format, &mut source, &mut dest)?;

    let signed_text = String::from_utf8(dest.into_inner())
        .map_err(|err| Error::BadParam(format!("signed text is not UTF-8: {err}")))?;
    let tampered_text = signed_text.replace("Original", "Modified");
    let mut tampered = Cursor::new(tampered_text.into_bytes());

    let reader = Reader::from_shared_context(&context).with_stream(format, &mut tampered)?;
    let statuses = reader
        .validation_status()
        .expect("tampered text should produce validation status");
    assert!(
        statuses
            .iter()
            .any(|status| status.code() == validation_status::ASSERTION_DATAHASH_MISMATCH),
        "expected data hash mismatch, got {statuses:?}"
    );
